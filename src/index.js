 ...
 * Paline Init
 */
PalineFactory.prototype.init = function (option) {
    let root = this;
    root['config'] = option
    if (typeof document === 'undefined') {
        console && console.warn('Sorry, Paline does not support Server-side rendering.')
        return;
    }
    !!option && root._init();
    return root;
...
PalineFactory.prototype._init = function(){
    let root = this;
    try {
        let {
            lang,
        ...
        root.config.gotapiNotifierChannel = gotapiNotifierChannel || ("/Paline-comment/notifier/"+id)
        root.config.notifyMail = root.config.notifyMail ||"";
        ...
        root.config.secret  = root.config.secret ||"";
        if (!id || !key) throw 99;
        ...
        // get comment count
        let els = Utils.findAll(document, '.Paline-comment-count');
...
PalineFactory.prototype.QueryReplies  = (path,rids,config)=>{
    let kiwi = new Kiwi(config);
    return kiwi.queryReplies(path,rids);
}
PalineFactory.prototype.QueryMain = (path,size,jump,config)=>{
    let kiwi = new Kiwi(config);
    return kiwi.queryMain(path,size,jump);
}
PalineFactory.prototype.queryCount = (path,config)=>{
    return (new Kiwi(config)).count(path);
}
PalineFactory.prototype.ErrorHandler = function (ex,origin) {
 ...
 */
PalineFactory.prototype.installLocale = function (locale, mode) {
    let root = this;
    mode = mode || {};
    if (locale) {
 ...
 * @param {String} path
 */
PalineFactory.prototype.setPath = function (path) {
 ...
 * Bind Event
 */
PalineFactory.prototype.bind = function (option) {
    let root = this;
    ...
    // cache
    let getCache = () => {
        let s = _store && _store.PalineCache;
        if (s) {
            s = JSON.parse(s);
            let m = meta;
            for (let i in m) {
    ...
    let commitEvt = () => {
        ...
        comment.save(root.config).then(ret => {
            setTimeout(()=>{     try{
                pida.post("https://ws.gotapi.net" + root.config.gotapiNotifierChannel,{
                    "headers":{
                        "secret":root.config.secret
                    }
                },{
                    type:2,
            ...
            if(root.config.notifyMail&&root.config.secret) {
                let mtitle = "您有新的评论"
                let mContent = "您有新的评论了，参见:!["+location.href+"]("+location.href+")"
                let mData = "to="+encodeURIComponent(root.config.notifyMail)+"&title="
                + encodeURIComponent(mtitle)+"&content="+encodeURIComponent(mContent)
                setTimeout(() => {
                    try {
                        pida.post("https://gotapi.net/v3/api/mail/send.notify",{
                            "headers":{
            ...
            defaultComment['nick'] !== 'Anonymous' && _store && _store.setItem('PalineCache', JSON.stringify({
...
function Paline(options) {
    return new PalineFactory(options)
}
export  default Paline;